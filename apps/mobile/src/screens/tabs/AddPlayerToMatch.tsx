...
export default function AddPlayerToMatch() {
  ...
  const currentPlayersId = [...bowlingTeamPlayers, ...battingTeamPlayers].map(
    (item) => item.id
  );
  console.log({ currentPlayersId });
  const remainPlayers = players.filter(
    (item) => !currentPlayersId.includes(item.id)
  );

  const battingTeam = match[battingRef];
  const bowlingTeam = match[bowlingRef];

  const [addPlayerPopup, setAddPlayerPopup] = useState({
    visible: false,
    title: '',
  });
  ...
  const handleAddPlayer = (plr) => {
    setAddPlayerPopup({ visible: false } as any);
    if (addTo === battingRef) {
      battingTeam.players.push(plr);
      dispatch({
        type: ADD_PLAYER_TO_MATCH,
        value: { [battingRef]: battingTeam },
      });
    }
    if (addTo === bowlingRef) {
  ...
  return (
    <View>
      ...
      <Popup
        visible={addPlayerPopup.visible}
        id={[]}
        title={addPlayerPopup.title}
